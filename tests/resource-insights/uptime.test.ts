...
// ---------------------------------------------------------------------------
{
  ...
  const node = createMockNode({
    ...
    name: 'Vmiss',
    online: false,
    time: '2026-09-22T09:20:00Z',
    updated_at: '2026-09-22T09:47:00Z', // Client metadata updated just now
    uptime: 0,
  })
...
// ---------------------------------------------------------------------------
// Section 42: Case 1 — node.time vs updated_at (refreshing client metadata must NOT reset downtime)
// ---------------------------------------------------------------------------
{
  console.log('\n[Section 42] Case 1 — node.time vs updated_at')
  const testNow = new Date('2026-09-22T09:47:00Z')
  // Node went offline at 09:27 (20 min ago)
  const node = createMockNode({
    uuid: 'vmiss-meta-refresh',
    name: 'Vmiss',
    online: false,
    time: '2026-09-22T09:27:00Z', // Agent heartbeat: 20 min ago
    updated_at: '2026-09-22T09:47:00Z', // Client metadata refreshed just now!
    uptime: 0,
  })
  ...
  const series: NormalizedMetricSeries = {
    ...
    retentionDays: 30,
    points: [
      { time: '2026-09-22T08:00:00Z', value: 5.0, count: 60 },
      { time: '2026-09-22T09:00:00Z', value: 5.0, count: 60 },
    ],
  }

  const res = calculateNode30dUptime(node, { kind: 'metrics', series }, testNow)
  // Heartbeat was 09:27. With 60s grace, offline starts at 09:28.
  // 09:28 to 09:47 is 19 minutes = 1140 seconds of confirmed downtime!
  assert.ok(res.uptimeRatio !== null && res.uptimeRatio < 1.0, `Uptime must be < 100%, got ${res.uptimeText}`)
  assert.notStrictEqual(res.uptimeText, '100.00%')
  assert.ok(res.diagnostics!.offlineSeconds >= 1100, `Confirmed offline seconds should be >= 1100, got ${res.diagnostics!.offlineSeconds}`)
  console.log(`✓ Section 42 Case 1 passed: updated_at does not reset offline calculation, uptime remains ${res.uptimeText}`)
}

// ---------------------------------------------------------------------------
// Section 42: Case 2 — fill_empty 30d with 1d actual active metric (firstActiveIdx)
// ---------------------------------------------------------------------------
{
  console.log('\n[Section 42] Case 2 — fill_empty 30d with 1d actual active metric')
  const node = createMockNode({ name: 'Fill-Empty-1d-Node', online: true })
  const startTime30d = now.getTime() - SECONDS_30_DAYS * 1000

  // 30 days of hourly points (720 points) generated by fill_empty=true
  // First 29 days (696 points) have value=null, count=0 (synthetic empty)
  // Last 1 day (24 points) has value=10, count=60
  const points: NormalizedMetricPoint[] = []
  for (let h = 0; h < 720; h++) {
    const t = new Date(startTime30d + h * 3600 * 1000).toISOString()
    if (h < 696) {
  ...
  const series: NormalizedMetricSeries = {
    ...
    metricKey: 'cpu.usage',
    intervalSeconds: 3600,
    retentionDays: 1, // Komari default retention 1 day
    points,
  }

  const res = calculateNode30dUptime(node, { kind: 'metrics', series }, now)
  assert.strictEqual(res.status, 'partial')
  // Coverage must reflect ~1 day, NOT 30 days!
  assert.strictEqual(res.coverageText, '覆盖约 1.0 / 30 天')
  assert.notStrictEqual(res.coverageText, '30 / 30 天')
  assert.strictEqual(res.uptimeText, '100.00%')
  console.log(`✓ Section 42 Case 2 passed: fill_empty synthetic points do not fake 30-day coverage (${res.coverageText})`)
}
